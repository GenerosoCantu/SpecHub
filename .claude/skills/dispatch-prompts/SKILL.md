...
description: Dispatch generated implementation prompts from the spec hub (WORKFLOW.md Step 3) — runs each PROMPT-{service}-{feature}.md as a headless Claude Code session in its own git worktree of the target repo, all in parallel, via scripts/dispatch.sh. Use when the user says "dispatch", "implement the prompts", "run the prompts", "step 3", asks to resume or fix a dispatched prompt, or asks to mark a prompt verified.
context: fork
agent: hub-ops
---

> Runs forked in the `hub-ops` subagent (Sonnet) so the orchestration never enters the main session's context. Request from the user: **$ARGUMENTS** (if empty: dispatch every `Generated` prompt).
...
For each prompt the script: creates `feature/...` from the base branch (`BASE_BRANCH` in `spechub.conf`) in its own worktree (`<repo>-worktrees/<branch>`), copies gitignored local files (`WORKTREE_COPY_FILES`), links dependency dirs (`WORKTREE_LINK_DIRS`), pre-registers the worktree as trusted in `~/.claude.json`, runs `claude -p` there with the header's **Recommended model**, permission mode `acceptEdits`, and the pre-approved tool allowlist, then flips the header to `Applied` and appends a **Dispatch Run Report** section to the prompt file. Logs land in `.dispatch/runs/` (gitignored).
...
Optional knobs (environment variables): `DISPATCH_CLAUDE_BIN`, `DISPATCH_ALLOWED_TOOLS`, `DISPATCH_DEPS=install` (run the service's install command instead of linking), `DISPATCH_MAX_TURNS`, `DISPATCH_SERVE=affected|none` (restart only the affected services, or nothing). Only change them when the user asks or a run failed because of them.
...
- Result (`ok` / error), model, turns and cost
