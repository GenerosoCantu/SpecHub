...
#
# Runs implementation prompts from the spec hub as headless Claude Code sessions,
# each inside its own git worktree of the target service repo, all in parallel.
#
...
#
# Environment overrides (spechub.conf supplies BASE_BRANCH, WORKTREE_COPY_FILES, WORKTREE_LINK_DIRS):
#   DISPATCH_CLAUDE_BIN        claude binary (default: `claude` on PATH, else the VS Code extension binary)
#   DISPATCH_BASE_BRANCH       base branch for feature branches (default: BASE_BRANCH from spechub.conf, else main)
#   DISPATCH_PERMISSION_MODE   permission mode for headless sessions (default: acceptEdits)
#   DISPATCH_ALLOWED_TOOLS     comma-separated pre-approved tools for headless sessions (see DEFAULT_ALLOWED_TOOLS)
...
#   DISPATCH_TRUST             1 | 0 — pre-register each worktree as trusted in ~/.claude.json (default: 1);
...
#   DISPATCH_MAX_TURNS         cap on agent turns per run (default: 150; set to 0 for unlimited)
...
find_claude() {
  if [ -n "${DISPATCH_CLAUDE_BIN:-}" ]; then
    [ -x "$DISPATCH_CLAUDE_BIN" ] || die "DISPATCH_CLAUDE_BIN is not executable: $DISPATCH_CLAUDE_BIN"
    echo "$DISPATCH_CLAUDE_BIN"; return
  fi
  ...
  die "claude CLI not found. Install it (npm i -g @anthropic-ai/claude-code) or set DISPATCH_CLAUDE_BIN."
}
...
# "Sonnet — reason" → sonnet. Falls back to sonnet when unrecognized.
model_alias() {
  local raw first
  raw="$(header_field "$1" "Recommended model")"
  first="$(printf '%s' "$raw" | awk '{print tolower($1)}')"
  case "$first" in
    haiku|sonnet|opus) echo "$first" ;;
    *) log "Unrecognized model tier '$raw' — defaulting to sonnet"; echo sonnet ;;
  esac
}
...
# A worktree is a new project path to Claude Code; without a trust entry the headless
...
Rules:
- The repo's instruction file (CLAUDE.md) in this directory is already in your context; follow it. Do not re-read it.
...
run_headless() {  # run_headless <worktree> <model> <log-json> <log-txt> [--resume <sid>] -- <prompt-text-file> <system-preamble>
  ...
  local resume_args=()
  if [ "${1:-}" = "--resume" ]; then resume_args=(--resume "$2"); shift 2; fi
  [ "${1:-}" = "--" ] && shift
  local prompt_text_file="$1" preamble="$2"
  local claude_bin; claude_bin="$(find_claude)"
  local turns_args=()
  [ -n "$MAX_TURNS" ] && turns_args=(--max-turns "$MAX_TURNS")
  ...
  (
    ...
    env -u CLAUDECODE -u CLAUDE_CODE_ENTRYPOINT \
      "$claude_bin" -p \
        ...
        < "$prompt_text_file" > "$out_json" 2> "$out_txt"
  )
}
...
summarize_run() {  # summarize_run <out_json> -> sets RUN_SID RUN_COST RUN_TURNS RUN_ERR RUN_RESULT_TAIL
  ...
  [ -n "$RUN_SID" ] || { RUN_SID="(none)"; RUN_ERR="true"; }
}
...
diagnose() {  # diagnose <rc> <out_json> <out_txt>
  ...
  if [ ! -s "$json" ]; then
    ...
    if [ -n "$other" ]; then echo "claude exited $rc without a result: $other$note"
    else echo "claude was killed before producing a result (exit $rc) — the process that launched it ended, or it was stopped by hand$note"; fi
    return
  fi
  ...
  case "$sub" in
    ...
    *) echo "claude reported an error (exit $rc, subtype ${sub:-?}): $(json_field "$json" result | head -c 200)$note" ;;
  esac
}
...
"| Result | ABORTED — $why |
| Diagnosis | the session never returned; its transcript is in ~/.claude/projects/ under the worktree path |
...
cmd_run() {
  ...
  for f in "${files[@]}"; do
    ...
    if [ "$DRY_RUN" = "1" ]; then
      printf 'DRY RUN  %s\n  repo:     %s\n  branch:   %s (from %s)\n  worktree: %s\n  model:    %s\n  perms:    %s + allowedTools\n' \
        "$(basename "$f")" "$repo" "$branch" "$BASE_BRANCH" "$wt" "$model" "$PERMISSION_MODE"
      continue
    fi
    case "$seen_repos" in *"|$repo|"*) ;; *) ensure_base_branch "$repo"; seen_repos="$seen_repos|$repo|" ;; esac
    setup_worktree "$repo" "$branch" "$wt"
  done
  [ "$DRY_RUN" = "1" ] && { log "Dry run — nothing was changed. claude: $(find_claude)"; return 0; }
  find_claude >/dev/null
...
cmd_resume() {
  ...
  reap_dead
  local sid; sid="$(grep -E '^\| Session ID \|' "$file" | tail -1 | sed -E 's/.*`([^`]+)`.*/\1/' || true)"
  [ -n "$sid" ] && [ "$sid" != "(none)" ] || die "No session ID recorded in $(basename "$file") — dispatch it with 'run' first."
