...
1. Reads the dispatch header: target repo (a git root), service, branch, recommended model tier.
2. In the target repo, creates the branch **from the base branch** (`BASE_BRANCH` in `spechub.conf`) and checks it out in its own git worktree at `<repo>-worktrees/<branch>`, so parallel prompts on the same repo never share a working tree and the repo's main checkout is untouched. A monorepo service runs from the same relative folder inside the worktree; the session itself runs from the worktree root and is told which subtree it owns. Gitignored local files (`WORKTREE_COPY_FILES`) are copied in, dependency directories (`WORKTREE_LINK_DIRS`) are linked from the main checkout, and, for Claude Code, the worktree path is pre-registered as trusted in `~/.claude.json` so the headless session does not die on the trust dialog.
...
|----------|-----------|---------|
| Service identifier | the `name` column of `spechub.conf`; its `dir` is a git root or a folder inside a monorepo | `api`, `web`, `billing` |
