...
**Then hold the spec files — still before any spec edit.** Run `scripts/lock.sh claim <feature> <spec-file>...` with the feature's kebab name (the `{name}` of its `FEATURE-`/`BUG-` file) and every spec / module file this cascade will edit — hub paths such as `02-api/orders.md`, or `api/orders`; a new module file may be named before it exists. The design session posted a coarse **designing** row for this feature; the claim upgrades it to **held** on exactly these files (or extends the hold when run again), as its own commit pushed to origin. If it refuses because another feature *holds* a file, stop and report the holder verbatim: nothing is cascaded over a held file, and no prompt is generated; the user coordinates with the holder or releases that row. A warning that another feature is merely *designing* against a file does not stop you — name it in the report so that designer knows to re-validate. Class A bugs edit no spec: release the intent instead (`scripts/lock.sh release <bug-name>`).
