...
Your first action, before reading a spec: post the design intent — `scripts/lock.sh intend <feature-name> <service-or-spec>...` with the kebab name the feature file will have and the services the design is expected to touch (coarse is fine; run it again when the design reveals more). It goes through origin like a feature number, so other instances see it. It is a notice, not a lock: it warns, never refuses. Report every overlap it prints to the user before designing (someone designing the same area, or a file held by a feature between cascade and close-out — a design against a held file must be re-validated once that hold is released). Then read:
- `WORKFLOW.md`
...
Post the intent first (`scripts/lock.sh intend <bug-name> <service>...`, as for a feature), read `WORKFLOW.md` → Bugs, then the relevant service spec (index + module file(s)). Create `Features/BUG-{name}.md` (`templates/BUG-TEMPLATE.md`), classed A (code diverges from spec — no spec edit) or B (spec wrong or silent — cascaded like an enhancement). A defect in a prompt that is still `Applied` is not a bug file: resume that prompt (`scripts/dispatch.sh resume`).

### Cascading a designed feature and generating its implementation prompts (Step 2)
Use the `cascade-and-prompt` skill. One session does both halves in order, without stopping between them: 2a adds the `STATUS.md` row, holds every spec / module file it is about to edit (`scripts/lock.sh claim` — upgrades the design intent to a hold on the exact files; stops when another feature holds one) and cascades the feature file into them; 2b re-reads the cascaded spec from disk and generates the prompts from it, including the mandatory dispatch header (target repo, branch, prerequisites, status, model tier). Never generate a prompt before the spec edit is on disk.
...
- `LOCKS.md`: spec locks board — one row per in-flight feature, state **designing** (Step 1 notice, blocks nobody) or **held** (Step 2a to close-out, refuses another hold on the same file), naming the spec areas, who and since when; written only by `scripts/lock.sh` (`intend` at Step 1, `claim` at 2a, `release` at Step 4 or staling), read with `scripts/lock.sh list`
...
- `templates/`: fill-in templates (architecture overview, conventions, status board, spec locks board, service spec, spec index, module file, feature file, bug file, prompt)
...
- `scripts/lock.sh`: `LOCKS.md` spec locks — `intend <feature> <spec>...` (Step 1, the design session's first action) posts a **designing** row and warns about overlaps, refusing nothing; `claim <feature> <spec>...` (Step 2a, before any spec edit) upgrades it to **held** on the exact module files, or extends a hold, and refuses while another feature holds one (a whole-service row covers every module under it); both go to origin as their own commit; `release <feature>` drops the row (`--no-push` at close-out, so it leaves with the close-out commit); `check` fails when two held rows share a file, here or on origin, and warns on design overlaps, stale or orphaned rows; `list` prints the board. Advisory: nothing stops an edit, the skills stop at the check
